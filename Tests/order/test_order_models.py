...
def test_order_item(order_item):
    ...
    assert (
        str(order_item)
        == f"order: {order_item.order.id} - {order_item.order.client.name} - {order_item.order.client.stock.sector.name} - {order_item.order.client.stock.sector.public_defense.name} - {order_item.product}"
    )
